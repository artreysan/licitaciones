...
/*Termina pregunta si ha tenido otros tratamientos*/
  ...
  // Opciones preguntas
  $('input[name=\"t3_pregunta1_opc\"]').click(function() {
    var v = accounting.unformat($(this).val());
    // console.log(v);

    $("#cont_t3_pregunta2_text").hide();
    if(v==1){
      $("#cont_t3_pregunta2_text").show();
    }else{
      $("#t3_pregunta2_text").val("");
    }
  });

  $('input[name=\"t3_pregunta4_opc\"]').click(function() {
    var v = accounting.unformat($(this).val());
    // console.log(v);

    $("#cont_t3_pregunta4_text").hide();
    if(v==1){
      $("#cont_t3_pregunta4_text").show();
    }else{
      $("#t3_pregunta4_text").val("");
    }
  });
  ...
  // Imp. 09/06/20
  $("#dp_tipos_facial").on("change", function(){
    var tipoFacial = accounting.unformat($(this).val());
    console.log(tipoFacial);

    if(tipoFacial>0){
      $("#dp_tecnologia").val("");
      $("#dp_tecnologia").addClass("sel_sololectura");
    }
  });

  $("#dp_tecnologia").on("change", function(){
    var tecnologia = accounting.unformat($(this).val());
    console.log(tecnologia);

    if(tecnologia>0){
      $("#dp_tipos_facial").val("");
      $("#dp_tipos_facial").addClass("sel_sololectura");
    }
  });

  $("#btn_reset_tipotec").on("click", function(){
    $("#dp_tipos_facial").val("");
    $("#dp_tecnologia").val("");
    $("#dp_tecnologia").removeClass("sel_sololectura");
    $("#dp_tipos_facial").removeClass("sel_sololectura");
    tratRecFaciales();
  });



  $("#ds_protocolo").change(function(){
    //console.log($(this).val());
    $("#ds_ids_protocolo").val($(this).val());
  });
  $("#ds_producto").change(function(){
    //console.log($(this).val());
    $("#ds_ids_productos").val($(this).val());
  });
  ...
  $("#nuevaSesion").click(function(){
    $("#formularioSesiont3").toggleClass('ocultar');
    $("#btn_salvar_expclientet3").toggleClass('ocultar');
  ...
  // Initialize the jQuery File Upload plugin
    $('#formExpcliente').fileupload({
        url: '../ajaxcall/ajaxFunctions.php?funct=guardarSesionest3',
  ...
  $("#formExpclientet3").change(function(){
    //console.log("Entre a realizar un cambio");
    if (crtSalvar == 0 && $("#idExpSesion").val() == 0){
      var urlDel = '../ajaxcall/ajaxFunctions.php?funct=guardarSesionest3';
      var formElement = document.getElementById("formExpclientet3");
  ...
  $("#btn_salvar_expclientet3").click(function(){
    salvarExpClientet3();
  });
...
function salvarExpClientet3(){
  $.validator.setDefaults({
    ignore: []
  });

  var validator = $("#formExpclientet3").validate({
  ...
  if($("#formExpclientet3").valid()){
    ...
    if(firma === "{\"lines\":[]}"){
      ...
      // Mandar post
      var arrParams = submitNameAbase64("#formExpclientet3");
...
/*function editarExpSesion(idExpSesion){
  ...
  if(idExpSesion > 0){
    ...
    ajaxData(params, function(data){
      ...
      $("#ds_ids_protocolo").val(datosJson['ds_ids_protocolo']);
      $("#ds_ids_productos").val(datosJson['ds_ids_productos']);
      ...
      ids_protocolo = (typeof(datosJson['ds_ids_protocolo']) != "undefined" && datosJson['ds_ids_protocolo'] !== null) ? datosJson['ds_ids_protocolo'].split(",") : "";
      ids_productos = (typeof(datosJson['ds_ids_productos']) != "undefined" && datosJson['ds_ids_productos'] !== null) ? datosJson['ds_ids_protocolo'].split(",") : "";

      if(typeof(datosJson['ds_ids_protocolo']) != "undefined" && datosJson['ds_ids_protocolo'] !== null){
        $.each(ids_protocolo, function (ind, elem) { 
          $("#ds_protocolo option[value="+elem+"]").prop("selected",true);
        }); 
        if(data.infoSesion['estatus'] == 1){
          $("#ds_protocolo").attr("disabled","disabled");
        }
        $(".selector").formSelect();
      }
      if(typeof(datosJson['ds_ids_productos']) != "undefined" && datosJson['ds_ids_productos'] !== null){
        $.each(ids_productos, function (ind, elem) { 
          $("#ds_producto option[value="+elem+"]").prop("selected",true);
        }); 
        if(data.infoSesion['estatus'] == 1){
          $("#ds_producto").attr("disabled","disabled");
        }
        $(".selector").formSelect();
      }
...
/*function mostrarCtrlsExt(noSesionNew, noSesionEdit, imagenes){
  var arraySesiones = ['1','5','10','15','20','25','30','35','40','45','50','55','60','65','70'];
  $.each(arraySesiones, function (ind, elem) { 
    ...
    if(noSesionEdit == elem){
      // Validar si el campo imagenes esta vacio mostrar plugin para subir imagenes
      if(imagenes == ""){
        $("#subirImagenes").show();
        $("#divImagenes").hide();
      }
      // Si no esta vacio mostrar imagenes cargadas
      else{
        $("#subirImagenes").hide();
        $("#divImagenes").show();
      }
...
function obtSesionesTipoFacial(){
  var tipoFacial = accounting.unformat($('#dp_tipos_facial').val());
  var sesiones = 0;

  if(tipoFacial==1 || tipoFacial==2){
    sesiones = 5;
  }

  if(tipoFacial==3 || tipoFacial==4 || tipoFacial==5){
    sesiones = 6;
  }
  return sesiones;
}

function tratRecFaciales(){
  var tipoFacial = accounting.unformat($('#dp_tipos_facial').val());
  var tecnologia = accounting.unformat($('#dp_tecnologia').val());
  ...
  //Obtener el numero de sesiones
  var sesiones = obtSesionesTipoFacial();

  // Obtener costo por tipo de tratamiento
  var costoZonas = obtCostoTotalZonaDiag();
  console.log(costoZonas);

  //Obtener costo po tecnologia
  var costoTec = obtCostoTecnologia();
  console.log(costoTec);
  if(costoTec>0){
    costoZonas = costoZonas+costoTec;
    // Suma 1 sesion en caso de que tenga seleccionado alguna tecnologia
    sesiones = sesiones+1;
  }
  console.log(costoZonas);
  ...
  $("#pro_duracion").val(sesiones+" sesiones");
  $("#pro_duracion_cont").val(sesiones);
  $("#pro_final_cont").val( ($("#pro_final_cont").val() !="")?$("#pro_final_cont").val():sesiones );
...
function obtCostoTotalZonaDiag(){
  ...
  $("#dp_tipos_facial option").each(function(i,v){
    if($(v).is(':selected')){
...
// Obt costo por tecnologia
function obtCostoTecnologia(){
  var precio = 0;

  $("#dp_tecnologia option").each(function(i,v){
    if($(v).is(':selected')){
      precio += accounting.unformat($(v).attr("precio"));
      // console.log(precio);
    }
...
function calculaTotalDescuento(ctr){
  ...
  if(ctr>0){
    let descMax = 0;
    if(tipoPago==0 || tipoPago==4){ descMax = 15; }
...
function guardarPropuesta(){
  ...
  if($("#formMotivoPropuesta").valid()){
    ...
    //obtener que tipo de tratamiento es
    //Imp. 24/06/20
    var dp_tipos_facial = accounting.unformat($("#dp_tipos_facial").val());
    var dp_tecnologia   = accounting.unformat($("#dp_tecnologia").val());
    // console.log(dp_tipos_facial);
    // console.log(dp_tecnologia);
    // return false;
  ...
  */
    ...
    var params = {idMotivo:id_motivopro, comentario:comentario, correoUsuario:correoUsuario, //Datos motivo y generar resumen
                  ...
                  numPagosPeriodo:numPagosPeriodo, dp_tipos_facial:dp_tipos_facial, dp_tecnologia:dp_tecnologia
                };
    // console.log(params);
    // return false;
    params = paramsB64(params);
    ...
    ajaxData(params, function(datosResp){
      ...
      if(datosResp.success==true){
        ...
        setTimeout(function(){
          ...
          window.open('propuestat3.php?idP='+idProspecto+'&c='+dp_email, '_blank', 'height=' + screen.height + ',width=' + screen.width + ',menubar=no,toolbar=no');
...
function pdfContrato(){
  ...
  var nombreTratamiento = "Faciales";
  ...
  params = paramsB64(params);
  params['funct'] = 'generarConsentimientoT3PDF';
...
function pdfContratoEjemplo(){
  setTimeout(function(){
    ...
    var archivoContrato = "";
    archivoContrato += '<div>Se ha generado el consentimiento, de click en el archivo <b><a href="../consentimiento_informado_faciales.pdf" target="_blank">Consentimiento</a></b> para que el usuario lo lea y lo firme, posterior a esto presionar sobre el bot&oacute;n de confirmar.</div>';
