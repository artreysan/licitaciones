...
/*Termina pregunta si ha tenido otros tratamientos*/
  ...
  /*$("#nuevaSesion").click(function(){
    ...
    $("#formularioSesiont4").toggleClass('ocultar');
    $("#btn_salvar_expclientet4").toggleClass('ocultar');
  ...
  $("#formExpclientet4").change(function(){
    //console.log("Entre a realizar un cambio");
    if (crtSalvar == 0 && $("#idExpSesion").val() == 0){
      var urlDel = '../ajaxcall/ajaxFunctions.php?funct=guardarSesionest4';
      var formElement = document.getElementById("formExpclientet4");
  ...
  $("#btn_salvar_expclientet4").click(function(){
    salvarExpClientet4();
  });
...
function salvarExpClientet4(){
  $.validator.setDefaults({
    ignore: []
  });

  var validator = $("#formExpclientet4").validate({
  ...
  if($("#formExpclientet4").valid()){
    ...
    if(firma === "{\"lines\":[]}"){
      ...
      // Mandar post
      var arrParams = submitNameAbase64("#formExpclientet4");
...
function obtSesionesTipoDiag(){
  var tipoDiag = accounting.unformat($('input:radio[name="t4_opc_diagnostico"]:checked').val());
  // console.log("tipoDiag: "+tipoDiag);
  var sesiones = 5;

  return sesiones;
}

function tratRecMasajes(){
  var tipoDiag = accounting.unformat($('input:radio[name="t4_opc_diagnostico"]:checked').val());
  // console.log("tipoDiag: "+tipoDiag);

  //Obtener el numero de sesiones
  var sesiones = obtSesionesTipoDiag();
    ...
    ajaxData(params, function(datosResp){
      ...
      if(datosResp.success==true){
        ...
        setTimeout(function(){
          ...
          window.open('propuestat4.php?idP='+idProspecto+'&c='+dp_email, '_blank', 'height=' + screen.height + ',width=' + screen.width + ',menubar=no,toolbar=no');
...
function pdfContrato(){
  ...
  var params = {idProspecto:idProspecto, nombreCompleto:nombreCompleto, correo:correo, edad:edad, telefono:telefono,            celular:celular,
                ...
                cf_nombre:cf_nombre, cf_parentesco:cf_parentesco, cf_telefono:cf_telefono,
                areas:areas, sesiones:sesiones, costoTotal:costoTotal
              };
   // console.log(params);
   // return false;

  params = paramsB64(params);
  params['funct'] = 'generarNotaCompraPDF';
  ...
  ajaxData(params, function(datosResp){
    ...
    if(datosResp.success==true){
      ...
      //
      var archivoContrato = "";
      archivoContrato += '<div>Se ha generado la nota de compra, de click en el archivo <b><a href="'+datosResp.urlArchivo+'" target="_blank">'+datosResp.nombreArchivo+'</a></b> para que el usuario lo lea y firme la nota de compra, posterior a esto presionar sobre el bot&oacute;n de confirmar.</div>';
      ...
      $('.modal').modal("close");
      alertify.error("No se ha podido generar la nota de compra, intentar nuevamente.");
    }
  });
}
